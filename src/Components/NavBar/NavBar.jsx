            ...
            <div className="navbar-start">
                <div className="dropdown">
                    ...
                    <div tabIndex={0} role="button" className=" mr-1 lg:hidden">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 btn-ghost p-[px] rounded" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h8m-8 6h16" /></svg>
                    </div>
            ...
            <div className="navbar-end">
                {
                    loading ? <h1> <span className="loading loading-spinner text-primary"></span> </h1> : <div className="flex gap-1">
                        {user ? <div className="flex items-center gap-2"> <div className="tooltip tooltip-bottom" data-tip={user?.displayName}> {user.photoURL ? <img className=" w-[50px] h-[50px] rounded-full border-[1px] " src={user.photoURL} alt="" /> : <FaCircleUser className="text-3xl" />}  </div> <button onClick={signingOutFromFirebase} className="btn p-2 bg-[#00ff6e]">Log out</button>  </div> : <Link to={'/login'}> <button onClick={''} className="btn bg-[#00ff6e]">Log In</button> </Link>}
